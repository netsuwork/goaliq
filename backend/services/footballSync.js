...
const FOOTBALL_API = process.env.FOOTBALL_API_BASE || 'https://v3.football.api-sports.io';
...
/**
 * Fetch upcoming & live fixtures from external API and upsert into MongoDB.
 */
async function syncMatches() {
  if (!API_KEY) {
    console.log('⚠️  No Football API key — using seed data only.');
  ...
  for (const [leagueKey, league] of Object.entries(LEAGUES)) {
    try {
      const { data } = await axios.get(`${FOOTBALL_API}/fixtures`, {
        headers,
        params: { league: league.id, season: league.season, next: 10 },
      });
      ...
      for (const fixture of data.response || []) {
        ...
        const odds = fixture.odds?.[0]?.bookmakers?.[0]?.bets?.[0]?.values || [];

        const oddsMap = {};
        odds.forEach(o => {
          if (o.value === 'Home') oddsMap.home = parseFloat(o.odd);
          if (o.value === 'Draw') oddsMap.draw = parseFloat(o.odd);
          if (o.value === 'Away') oddsMap.away = parseFloat(o.odd);
        });
      ...
      console.log(`✅ Synced ${data.response?.length || 0} matches for ${league.name}`);
    } catch (err) {
  ...
  for (const [leagueKey, league] of Object.entries(LEAGUES)) {
    try {
      ...
      await Standing.findOneAndUpdate(
        ...
        { league: leagueKey, leagueName: league.name, season: String(league.season), table, updatedAt: new Date() },
        { upsert: true, new: true }
      );
      console.log(`✅ Synced standings for ${league.name}`);
    } catch (err) {
      console.error(`❌ Failed to sync standings for ${league.name}:`, err.message);
    }
  }
}
...
/**
 * Seed demo matches when no API key is present.
 */
